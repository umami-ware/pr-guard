...
export class PrGuard {
	...
	private async checkRuns(): Promise<'success' | 'pending' | 'failure'> {
		...
		console.log(
			`Jobs that will be checked (${checkRuns.length}): [${checkRuns
				.map((x) => `${x.name} - ${x.status} (${x.conclusion})`)
				.join(', ')}]`
		);
		...
		const pendingChecks = checkRuns.filter((x) => x.status != 'completed');

		console.log(
			`Completed jobs (${passedChecks.length}): [${passedChecks
				.map((x) => x.name)
				.join(', ')}]`
		);

		console.log(
			`Failed jobs (${failedChecks.length}): [${failedChecks
				.map((x) => x.name)
				.join(', ')}]`
		);

		console.log(
			`Pending jobs (${pendingChecks.length}): [${pendingChecks
				.map((x) => x.name)
				.join(', ')}]`
		);
